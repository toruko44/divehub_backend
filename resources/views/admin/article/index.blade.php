...
@section('content')
    ...
    ?>
    <?php
    $breadcrumb_items = [['記事管理', route('admin.article.index')]];
    ?>

    <x-admin-page-title>記事管理</x-admin-page-title>
    <x-admin-breadcrumb :items="$breadcrumb_items" />
    <div class="bg-white rounded-md shadow-lg border-cyan-500 border-t-2 p-6 m-4">
        <x-card-title>記事一覧</x-card-title>

        <form method="GET" action="{{ route('admin.article.index') }}" class="m-5">
                                ...
                                @elseif ($key === 'actions')
                                    <div class="flex justify-center">
                                        <a href="{{ route('admin.article.show', $item->id) }}"
                                            class="text-cyan-600 hover:text-cyan-400 mr-2">詳細</a>
